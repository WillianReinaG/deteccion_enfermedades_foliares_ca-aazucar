...
def find_best_model() -> Dict[str, Any]:
    ...
    matches = list(MODEL_DIR.rglob("best.pt")) + list(MODEL_DIR.rglob("*.pt"))
    if matches:
        return {"model_id": "regnet_y_3_2gf", "framework": "torch", "best_ckpt": str(matches[0])}
...
class SugarCanePredictor:
    ...
    def predict(self, image: Image.Image, with_gradcam: bool = True) -> Dict[str, Any]:
        ...
        if self.framework == "yolo":
            ...
            result = self._format(probs, names)
            result["gradcam_image"] = None
            result["gradcam_array"] = None
            result["gradcam_error"] = "Grad-CAM no está habilitado para modelos YOLO en esta implementación."
            return result
        ...
        if with_gradcam and gradcam_supported(self.model_id, self.framework):
            ...
            overlay = gradcam_result[0]
            err = gradcam_result[1]
            grayscale_cam = gradcam_result[2] if len(gradcam_result) > 2 else None

            result["gradcam_image"] = overlay
            result["gradcam_class"] = result["class_name"]
            result["gradcam_error"] = err
            result["gradcam_array"] = grayscale_cam
        else:
            result["gradcam_image"] = None
            result["gradcam_class"] = result["class_name"]
            result["gradcam_error"] = f"Grad-CAM no soportado para modelo={self.model_id}, framework={self.framework}"
            result["gradcam_array"] = None
    ...
    def _demo_predict(self) -> Dict[str, Any]:
        ...
        return {
            **self._format(probs, self.class_names),
            "demo_mode": True,
            "checkpoint": "No configurado",
            "gradcam_image": None,
            "gradcam_array": None,
            "gradcam_error": "Modo demo: Grad-CAM no disponible."
        }
